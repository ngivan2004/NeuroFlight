...
from mlp import FlightPricePredictor
    ...
    for batch_idx, (X_batch, y_batch) in enumerate(train_loader):
        ...
        if batch_idx % 100 == 0:
            print(f"Epoch [{epoch+1}/{EPOCHS}], Step [{batch_idx+1}/{len(train_loader)}], "
                  f"Loss: {loss.item():.4f}, Accuracy: {(correct_predictions / total_predictions) * 100:.2f}%")
            writer.add_scalar('Training/Loss', loss.item(),
                              epoch * len(train_loader) + batch_idx)
            writer.add_scalar('Training/Accuracy', (correct_predictions /
                              total_predictions) * 100, epoch * len(train_loader) + batch_idx)
    ...
    with torch.no_grad():
        for batch_idx, (X_batch, y_batch) in enumerate(test_loader):
            ...
            if batch_idx % 100 == 0:
                ...
                writer.add_scalar('Evaluation/Accuracy', (correct_predictions /
                                  total_predictions) * 100, epoch * len(test_loader) + batch_idx)
